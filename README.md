...
* **`js/dashboard.js`:** Fetches and renders the high-level metrics, active project lists, and global fixed costs for both agents and superusers. Includes categorized tab views (All, In Progress, Delivered, Completed, and Group B) for streamlined tracking. It ensures fixed costs are intentionally omitted (set to 0) in the "In Progress" and "Delivered" tabs.
